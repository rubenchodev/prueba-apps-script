/**
 * Función que permite convertir un valor numerico en letras
**/
var CifrasEnLetras = {
    ...
    palabraDecimal, palabraDecimalPlural, esFemeninaPalabraDecimal)
  {
    with (CifrasEnLetras) {
      ...
      if (cifrasDecimal != "") {
        ...
        resultado.push(" Pesos " + completeDecimal(cifrasDecimal) + "/100 M.N.");
      } else {
        resultado.push(" Pesos 00/100 M.N.");
      }
 
      return resultado.join("");
    }
  },
  completeDecimal: function(decimal){
    
    var stringDecimal = String(decimal);
    
    // validamos si la cantidad de numeros es menor que 2
    if(stringDecimal.length < 2){
      
      // se recorre la cantidad que falta
      for(var i = stringDecimal.length; i < 2; i++){
        
        // agregamos un cero al final
        stringDecimal += "0";
